...
void receiveCommand() {
  if (rs485Serial.available()) {
    String receivedString = rs485Serial.readStringUntil('\n');
    receivedString.trim();

    int pos = receivedString.indexOf("/");
    if (pos != -1) {
      String commandString = receivedString.substring(0, pos);
      String dataString = receivedString.substring(pos + 1);
      int command = commandString.toInt(); 

      /*if (command == 7) {
        Mess = dataString; // Mettez à jour la variable Mess avec la donnée reçue
        Fct_receptionRS485(Mess); // Envoyer la commande 7 avec la donnée à Fct_receptionRS485
      } else */
      if (command == 6) {
        dataString = "";
        Fct_receptionRS485(dataString); // Exécuter la commande 6 dans Fct_receptionRS485
      } else if (command >= 1 && command <= 60) {
        Fct_receptionRS485(Mess); // Exécuter la commande sur l'afficheur
      }
    }
  }
}
  ...
  switch (commande) {
    ...
    case 3://CPT VISITEUR P2 2 LIGNES
     ...
     while (commande==5){
            ...
            matrix.setTextColor(matrix.Color333(7, 7, 7));
            matrix.setCursor(textX, 0);
            matrix.print("Le pass parking+ est a -30%");
          // Move text left (w/wrap), increase hue
          if((--textX) < textMin){ textX = matrix.width(); i++;}
...
afficherAccueil();
    ...
    case 11:
         while (commande==11){
            ...
            matrix.setTextColor(matrix.Color333(7, 7, 7));
            matrix.setCursor(textX, 0);
            matrix.print("Le parking est ouvert de 8h a 20h");
          // Move text left (w/wrap), increase hue
          if((--textX) < textMin){ textX = matrix.width(); i++;}
    ...
    case 12:
         while (commande==12){
            ...
            matrix.setTextColor(matrix.Color333(7, 7, 7));
            matrix.setCursor(textX, 0);
            matrix.print("Le sous-sol est actuellement inonde");
          // Move text left (w/wrap), increase hue
          if((--textX) < textMin){ textX = matrix.width(); i++;}
    ...
    case 13:
         while (commande==13){
            ...
            matrix.setTextColor(matrix.Color333(7, 7, 7));
            matrix.setCursor(textX, 0);
            matrix.print("Le parking exterieur est inaccessible");
          // Move text left (w/wrap), increase hue
          if((--textX) < textMin){ textX = matrix.width(); i++;}
...
void afficherAccueil(){
 while (commande==10){
        ...
        matrix.print("Afficheur");  //F2(str)
         
        matrix.setTextColor(pgm_read_word(&Color[3]));
        matrix.setCursor(12, 9);
        matrix.print("General");  //F2(str)
